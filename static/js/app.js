async function setup() {
    const patchExportURL = "static/export/patch.export.json";
    ...
    let dependencies = [];
    try {
        const dependenciesResponse = await fetch("static/export/dependencies.json");
        dependencies = await dependenciesResponse.json();

        // Prepend "export" to any file dependenciies
        dependencies = dependencies.map(d => d.file ? Object.assign({}, d, { file: "static/export/" + d.file }) : d);
    } catch (e) {}
...
function makeSliders(device) {
    ...
    let uiElements = {};
    let targetParam = ["humidity", "temperature", "wind_speed"];
    ...
    device.parameters.forEach(param => {
        ...
        if(targetParam.includes(param.name)){
